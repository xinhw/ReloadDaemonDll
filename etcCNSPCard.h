...
#define READER_TYPE_D8				0x04			//	德卡D8读卡器
...
extern "C"
{
	...
	*/
	...
	/*29. 获取CPC卡的序列号*/
	int __stdcall cpcGetSNO(BYTE *szSNO,int ncom = gnDefaultCom);
	/*30. CPC卡初始化*/
	int __stdcall cpcInit(BYTE *elf01,int ncom = gnDefaultCom);
	/*31. 读取CPC卡信息*/
	int __stdcall cpcRead(BYTE *elf01_mf,BYTE *elf02_mf,BYTE *ef01_df,int ncom = gnDefaultCom);

	/*32. OBU通过SAM母卡初始化*/
	/*
		bSAMNode:	[in]	SAM卡节点号
		elf01_mf:	[in]	OBU系统信息文件，27字节
		elf01_adf:	[in]	OBU车辆信息文件，59字节
		ncom:		[in]	读卡器端口号
	*/
	int __stdcall obuInitBySAM(BYTE bSAMNode,BYTE *elf01_mf,BYTE *elf01_adf,int ncom = gnDefaultCom);
