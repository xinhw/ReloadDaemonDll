...
class CJYRsuReader : public CCardReader
{
public:
	CJYRsuReader(void);
	~CJYRsuReader(void);
private:
	...
	// RSU初始化
	//int RSU_INIT_rq(long fd, char *Time, int BSTInterval, int TxPower, int PLLChannelID, int TimeOut)
	typedef int (WINAPI *LPFN_RSU_INIT_rq)(long fd, char *Time, int BSTInterval,int TxPower, int PLLChannelID, int TimeOut);

	// RSU初始化返回
	//	int RSU_INIT_rs(long fd, int * RSUStatus, int * rlen, char *RSUinfo, int TimeOut);
	typedef int (WINAPI *LPFN_RSU_INIT_rs)(long fd, int * RSUStatus, int *rlen,char *RSUinfo, int TimeOut);
	
	// PSAM通道复位
	//	int PSAM_Reset_rq(long fd, int PSAMSlot, long baud, int TimeOut);
	typedef int (WINAPI *LPFN_PSAM_Reset_rq)(long fd, int PSAMSlot, long baud, int TimeOut);

	// PSAM通道复位返回
	// int PSAM_ Reset_rs (long fd, int PSAMSlot, int *rlen, char *Data, int TimeOut);
	typedef int (WINAPI *LPFN_PSAM_Reset_rs)(long fd, int PSAMSlot, int *rlen,char *Data, int TimeOut);

	// PSAM通道指令
	//	int PSAM_CHANNEL_rq(int fd, int PSAMSlot , int APDUList ,char *APDU, int TimeOut);
	typedef int (WINAPI *LPFN_PSAM_CHANNEL_rq)(int fd, int PSAMSlot , int APDUList ,char *APDU, int TimeOut);
	
	// PSAM通道指令返回
	//	int PSAM_CHANNEL_rs (int fd, int PSAMSlot , int * APDUList ,char *Data, int TimeOut);
	...
	// BST
	//		int INITIALIZATION_rq (long fd, char *BeaconID , char *Time,  int Profile, 
	...
	typedef int (WINAPI *LPFN_INITIALISATION_rq)(long fd, char *BeaconID , char *Time, int Profile,
							...
							int Profilelist, int TimeOut);
	// VST
	//	int INITIALIZATION_rs (long fd, int * ReturnStatus, int * Profile,
	...
	// 透传接口
	//	int TransferChannel_rq(long fd, int  mode, int DID, int ChannelID, 
	//						int APDULIST,  char *APDU, int TimeOut)
	typedef int (WINAPI *LPFN_TransferChannel_rq)(long fd, int mode, int DID, int ChannelID,
							int APDULIST, char *APDU, int TimeOut);

	// 透传应答
	//	int TransferChannel_rs(long fd, int DID,  int * ChannelID,  int * APDULIST,  char *Data,  int * ReturnStatus, int TimeOut);
	...
	// OBU休眠请求
	//	int SetMMI_rq(long fd, int  mode, int DID,  int SetMMIPara, int TimeOut);
	typedef int (WINAPI *LPFN_SetMMI_rq)(long fd, int mode, int DID, int SetMMIPara, int TimeOut);

	// OBU休眠应答
	//	int SetMMI_rs(long fd, int * DID, int * ReturnStatus, int TimeOut);
	typedef int (WINAPI *LPFN_SetMMI_rs)(long fd, int * DID, int * ReturnStatus, int TimeOut);

	//	发出读取OBU文件请求(GetSecure_rq)
	//	int GetSecure_rq (long fd, int accessCredentialsOp ,int mode, int DID, 
	//					 char * AccessCredentials,  int keyIdForEncryptOp, int FID, 
	//					 int offset, int length, char * RandRSU, int KeyIdForAuthen, 
